...
fun A2V (n,c) = ((n,Bot),c)   (* converts an assignment to the body of a Var *)
...
(* Conor *)
       | Ref b         => if (ref_isDefnNow b)
                          then (case (ref_kind b)
                                  of Voo _ => occ_rec (ref_val b)
                                   | _ => true)
                          else true
       | _             => true
    in occ_rec end
...
fun sub [] = I
  | sub s  =  
    let
      fun sub_rec (Var(b as ((n,_),t))) =
	   (Mod(assoc n s) handle _ => mkVar sub_rec b)
        | sub_rec (Ref b) =
          (case ref_bd b
             of ((Hole,Def),_,Var (x as ((n,_),t)),_) =>
                (Mod(assoc n s) handle _ => mkVar sub_rec x)
              | _ => UMod)
...
fun Unirec uniflg LMflg s (M,N) =
  let
	...
	fun reunion ((p,_),P) M =
	   ...
	   *)
          let fun ConorParanoia (M as Ref b) =
                  if (ref_isDefnNow b) andalso
                     ((fn Voo _ => true | _ => false) (ref_kind b))
                  then ConorParanoia (ref_val b)
                  else M
                | ConorParanoia M = M
              val M = ConorParanoia M
              fun earlyBath (Var ((q,_),_)) = p=q
                | earlyBath _               = false
          in  if earlyBath M then s else
              if (mv_occur p M) orelse (free_occur M) then
              let val M = UMnorm M
              in  if earlyBath M then s else
                  let val ov = free_occur M
                      val mv = mv_occur p M
                  in  if mv orelse ov
                      then raiseUnif (("reunion: "^
                            (if mv then "occurs check" else "free o-var")),
                            (Bot,Bot))
...
******************)
	case MN of
	  (Var(mtm as ((m,_),_)),Var(ntn as ((n,_),_))) =>
	    if m=n then s
...
fun whnf_unif s M N =
  let
	...
	val _ = if not(!whun_debug) then ()
      ...
      in
	case MN
	  of (Var(mtm as ((m,_),_)),Var(ntn as ((n,_),_))) =>
	        if m=n then s
		else if m>n then reunion mtm N
		     else reunion ntn M
