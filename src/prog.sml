        ...
        fun gdefn (qn,qT) defns =
           (let val (QC,QT) = introall iota "x" 1 ([],qT)
                ...
                val noc = case parseApp nfa
                            of (_,aa,_) => whnf (last aa)
                val (rhs,gl,av) =
                    case parseApp noc
                      of (Var ((n,_),_),aa,vv) => 
                         (Var ((n,Bot),Bot),SOME n,(aa,vv))
                       | _ => (mangle >> (UMnorm noc),NONE,([],[]))
   ...
   (let fun thud () = failwith "not a programming problem"
        val goals = case Namespace.getCurrentGoals ()
                      of [Unknown gs] => gs
                       | _ => thud ()
        val decls = findDecls goals
        val typeBs = map (fn (_,{fTucr=(tb,ub,cb,rb),...}) => tb) decls
        fun isTgoal (_,t) =
           (let val b = headRef t
            in  exists (sameRef b) typeBs
            end handle Match => false)
        val (Tgoals,_) = filter isTgoal goals
        val tac = tactic_wrapper (fn _ => let
        val _ = foldr (fn (g,_) => fn _ => Toplevel.Next g) () Tgoals
    in  () end)
    in  tac ()
    end handle _ => failwith "bad stuff" )
...
fun legoProgElim (rvc : cnstr_c) (name : string) =
  ( let fun thud () = failwith "not a programming problem"
        ...
        val nqb = noo (HoleBV,"paf") ("q",1) (QC,sub [(gn,Ref ngb)] QT)
        val qrb = noo ((Let,Def),"lego'") ("lego",qn) ([],Ref nqb)
        val S as (C,_) = on ([qrb,nqb,grb,ngb],Prop) [
                           progElim 1 rv tg
                         ]
        val (GC,CnoG) = filter (hasName "g") C
        val LGC = map holeldav GC
        val (PGC,PGT) = S ?! ("lego",gn)
        val gnom = mkNameLoc ("prog_goal"^(string_of_num gn))
        val nglb = noo ((Let,Def),gnom) ("goal",gn)
                       (PGC@GC,PGT)
        val gcpt = foldr (fn _ => fn t => App_c (ShowNorm,t,NewVar_c))
                         (Ref_c gnom) GC
        val (QC,_) = filter (hasName "q") CnoG
        val LQC = map holeldav QC
        val (PQC,PQT) = S ?! ("lego",qn)
        val qnom = mkNameLoc ("prog_goal"^(string_of_num qn))
        val nqlb = noo ((Let,Def),qnom) ("goal",qn)
                       (PQC@QC,PQT)
        val qcpt = foldr (fn _ => fn t => App_c (ShowNorm,t,NewVar_c))
                         (Ref_c qnom) QC
        val tac = tactic_wrapper (fn _ => let
        val _ = claimHolesSolveGoals [] (const []) ([nqlb,nglb],Prop)
        val _ = Toplevel.Refine qn 0 qcpt
        val _ = Toplevel.Refine gn 0 gcpt
        val _ = legoProgNexts ()
    in  legoProgDisplay () end)
    in  tac ()
    end handle _ => failwith "bad Program Elim" )
...
fun legoProgRefine (rhs : cnstr_c) =
    let fun thud () = failwith "not a programming problem"
        ...
        val (C,T) = on ([ngb],App ((Bot,[Ref ngb]),[ShowNorm])) [
                      progRefine ("g",1) aliases Vrhs
                    ]
        val C = map holeldav C
        val T = case parseApp T
                  of (_,T::_,_) => T
                   | _ => T
        val redf = noo ((Let,Def),"Sol"^(string_of_num gn)) ("sol",1) (C,T)
        fun tac _ = let
        val _ = claimHolesSolveGoals [] (const []) ([redf],Prop)
        val dnam = (ref_nam o hd o tl o fst) (start Prop)
        val rterm = foldr (fn _ => fn f => App_c (ShowNorm,f,NewVar_c))
                    (Ref_c dnam) C
        val _ = Toplevel.Refine gn 0 rterm
    in  legoProgDisplay ()
    end in tac ()
    end
...
fun legoProgAbst (abs_c : cnstr_c) =
   (let fun thud () = failwith "gang aft agley"
        ...
        val (C,T) = on ([unitb,clueb,solb,ngb,goalb,retb,callb,unitFb,
                         typeFb],Prop)
                      [progRefine ("g",1) aliases bokko,
                       voodom ("clue",2)]
        val (f,a1,a2,a3,a4,vv) = case parseApp T
                                   of (f,[a1,a2,a3,a4],vv) =>
                                      (f, a1,a2,a3,a4, vv)
                                    | _ => bug "Prog Abst"
        val (defb,a4') = shareHoles [mainb,goalb] ("Definition"^name) a4
        val T' = MkApp ((f,[a1,a2,a3,a4']),vv)
        val S = on (C,T') [
                  vooBeforeDom defb,
                  domvoo ("clue",2)
                ]
