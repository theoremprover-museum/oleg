...
struct
  ...
  fun manageVars() =
	...
	in (newVars:= !newVars@[nc]; Var(A2V nc))
	end
      fun eraseNewVars t = 
...
(* run Mark methods *)     (map (fn f => f ())
	...
	fun pushHistProven nvt = HIST:= Proven(nvt,!HIST)
        fun getFinishedProof () = case (!HIST)
                                    of Proven (x,_) => SOME x | _ => NONE
...
(*  val HIST = ref NoProof;
  ...
  fun search nam = 
    let fun fail() = failwith("search: undefined or out of context: "^nam)
      fun srec ((v as ref (Bd{bd=(_,n,_,_),...}))::l) =
	if n=nam then v
	else srec l
	| srec [] = fail()
    in srec
  ...
  fun extendCxt knd bv =
	...
	in if kind k (***** andalso Pure nam t *************)
	     then (ref (Bd{ts=timestamp(),frz=ref UnFroz,
	...
	let
	  ...
	  val vt2 = dnf vt2
	in
	  (ref (Bd{ts=timestamp(),frz=(ref frz_flg),param=par_flg,
  ...
  in
	...
	fun intros_rec count lst c push =
	  let
		...
		val tr = subst1 (Var (A2V goal_l)) tr
		val goal_r = mk_goal tr
		(***********  Tuple is unsafe ********)
		val sbst = [(n,Tuple(Bind sigBod,
                                    [Var (A2V goal_l),Var (A2V goal_r)]))]
	      in
...
fun init_namespace() = NSP:=[ref (Bd{ts=timestamp(),
				frz=ref UnFroz,
				param=Global,
				deps=[],
				kind=Bnd,
...
fun SaveReductGbl VT =
  let fun saveReduct (v,t) cxt =
    (ref (Bd{ts=timestamp(),frz=ref UnFroz,param=Global,deps=[],kind=Red,
	bd=((Sig,VBot),"",v,t)}))::cxt
  in NSP:= saveReduct VT (!NSP)
  end;
fun makeAllPats() =     (* where to put this? *)
  let fun addRed (ref (Bd{bd=(_,_,v,_),kind=Red,...})) = add_reductions v
	| addRed _ = ()
...
local
  ...
  fun dk (left,deltas,moved) br =
	...
	{kind=Bnd,ts,param=Global,deps=deps,bd=((Let,_),_,v,t),...} => 
	  ...
	  localDecl ts n (v,t)
      | {kind=Constructor _,ts,param=Global,deps=deps,bd=(_,_,v,t),...} =>
	  globalDecl ts deps (v,t) br
      | {kind=Constructor _,ts,param=Local,bd=(_,n,v,t),...} =>
	  localDecl ts n (v,t)
...
(* Conor's first attempt at added slyness ******************

local
  fun slyVis br (Bind ((Pi,_),_,dom,ran)) = if depends br dom then Hid
                                            else slyVis br ran
    | slyVis br (Bind (_,_,_,ran)) = slyVis br ran
    | slyVis br _ = Vis
  fun visHack v (Bind ((Lda,_),nl,dl,rl),Bind ((Pi,_),np,dp,rp)) =
                (Bind ((Lda,v),nl,dl,rl),Bind ((Pi,v),np,dp,rp))
    | visHack v (Bind ((Pi,_),np,dp,rp),T) =
                (Bind ((Pi,v),np,dp,rp),T)
    | visHack v x = x
  fun dk sly (left,deltas,moved) br =
    let
      val _ = (prs(" "^ref_nam br); flush_out std_out)
      fun discharge oper deps (vt as (v,t)) br_sbj =
	let
	  ...
	  fun operate (vcvd as (vt as (v,t),cs,vs,deps)) br =
	    (if !disch_debug
		 ...
		 then case ref_bind br
			of Lda => if sly
                                  then
                                  let val myVis = case v
                                       of Bind ((Pi,_),_,_,_) => slyVis br v
                                        | _ => slyVis br t
                                  in (visHack myVis (oper vt br),
				      (Ref br)::cs,
				      (prVis myVis)::vs,
				      filter_neg (sameNam br) deps)
                                  end
...
in
  fun dischg sly nam msg =
    if activeProofState() then failwith"No Discharge in proof state"
	   ...
	   val _ = prs("Discharge and "^msg^".. ")
	   val _ = init_history()
	   val res = foldl (dk sly) (old,[],[]) (hit::new)
	   val _ = message("\n   (* "^(makestring_timer t)^" *)")
         in  res
	 end
end;
...
local
  fun dischState nsp = (NSP:= nsp; line(); makeAllPats())
in
  fun DischargeKeep sly nam = let val (left,_,moved) = dischg sly nam "keep"
			  in  dischState (moved@left)
			  end
  and Discharge sly nam = let val (left,_,_) = dischg sly nam "forget"
		      in  dischState left
		      end
end;
...
local
  fun forget f nopMsg killMsg smsg =
	...
	| sr []        = failwith killMsg        
    in  (NSP:= sr (getNamespace()); makeAllPats(); message smsg)
    end
in
  fun Forget nam =
    forget (fn br => sameNam br nam) 
           ("Forget is no-op; \""^nam^"\" not in namespace")
	   ("Forget forced KillRef; \""^nam^"\" no longer in namespace")
	   ("forgot back through "^nam)
  fun ForgetMrk mrk =
    forget (fn br => sameMrk br mrk)
           ("ForgetMark is no-op; \""^mrk^"\" not in namespace")
	   ("bug at ForgetMark")
	   ("forgot back through Mark \""^mrk^"\"")
end;

fun addLabel tn =
    NSP := (ref (Bd{ts=timestamp(),frz=ref UnFroz,param=Global,
...
fun addConfig config
    ...
    fun findConfig key fail =
        let
            fun fc2 [] = fail
              | fc2 ((ref (Bd{kind=Config(a,b,c,d),...}))::tail) =
...
fun StartTheory nam =
  let
    fun addThry() = NSP := (ref(Bd{kind=StThry nam,
			       ts=timestamp(),
			       frz=ref UnFroz,
			       param=Global,
			       deps=[],
