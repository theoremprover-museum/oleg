...
(* Conor's heuristic for deciding conditional visibility *)
  fun arg_infer (Bind ((Pi,_),_,dom,ran)) br =
      depends br dom orelse arg_infer ran br
    | arg_infer (Bind ((Let,_),_,dom,ran)) br = arg_infer ran br (* apr 00 *)
    | arg_infer _ _ = false
(* This test hides too many arguments---dependency is a necessary
   but not sufficient condition to guarantee inferrability. *)
  ...
  fun abstract (V,T) br =
    let
      ...
      val vis = if vis = VBot
                then if arg_infer T br then Hid else Vis
                else vis
  ...
  fun generalize (V,T) br =
    let
      ...
      val ((_,vis),s,c,t) = ref_bd br
      val vis = if vis = VBot
                then if arg_infer V br then Hid else Vis
                else vis
      val typ =
	case (t,whnf T)
	  of (_,Prop)          => Prop
	 ...
	 of (ShowNorm,Bind((Pi,Hid),nam,dom,rng),_) =>
	  ...
	  | (pv,Bind((Pi,Vis),nam,dom,rng),_) =>
		...
		SOME(s) => ((MkApp((Vf,[Va]),[pv]),coerceGe (subst1 Va rng)),s)
	      | NONE => 
                (case whnf (sub sbst Ta) (* Conor's wheeze April 00 *)
                   of Bind ((Pi,v),nam,hdom,hran) => (* apply like refine! *)
                      let val varg = mkVar hdom
                          val newVa = App((Va,[varg]),[prVis v])
                      in  Apply sbst mkVar ShowNorm
                             (Vf,Tf) (newVa,coerceGe (subst1 varg hran))
                      end
                    | _ => raise Error.error (Error.APPLNTYPE,NONE,
